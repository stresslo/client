...
const Product = () => {
    ...
    const getProducts = async () => {
        try {
            ...
            !decode.data.length && setMessage(`- (${ctg}) this category is still empty -`)
            setData(decode.data)
        }   catch (error) {
    ...
    }, [page, value, filterHistory, location.pathname])
                                ...
                                <div className='product-card' key={index} onClick={() => navigate(`/product/details/${i.vid}`, {state: i})}>
                                    ...
                                    <div className='wrapped-text'>
                                        ...
                                        <div style={{ display: 'flex', flexWrap : 'wrap', flexDirection : 'column'}}>
                                            <div className='product-desc'>{i.desc ? i.desc.length >= 35 ? i.desc.substring(0,35) + '...' : i.desc : ''}</div>
                                            <div className='wrapdet' style={{ position: 'unset', marginTop: '15px', marginLeft: '5px', gap: '5px' }}>
                                                ...
                                                : 
                                                <div style={{ backgroundColor: 'var(--background)', width: '95px', height: '30px'}}>{i.price.split(' ')[0].split('.')[0]}k</div>
                                                }
                                             </div>
                                        </div>
